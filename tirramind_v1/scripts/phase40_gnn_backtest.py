...
def main() -> None:
    ...
    _p = _ap.ArgumentParser(description="Phase 40 GNN walk-forward backtest")
    _p.add_argument(
        "--model-path",
        type=Path,
        default=MODEL_PATH,
        help=f"Path to GNN checkpoint .pt file (default: {MODEL_PATH})",
    )
    _p.add_argument(
        "--db-path",
        type=Path,
        default=DB_PATH,
        help=f"Path to pipeline.db (default: {DB_PATH})",
    )
    _p.add_argument(
        "--out",
        type=Path,
        default=None,
        help="Write IC summary JSON here so auto_improve.py can read it",
    )
    _cli = _p.parse_args()
    _model_path: Path = _cli.model_path
    _db_path: Path = _cli.db_path
    _out_path: Path | None = _cli.out
    ...
    # Compact summary: best strategy by mean_ic + all per-strategy entries.
    # auto_improve reads _out_path or checkpoint_dir/ic_results.json.
    _ic_summary: dict = {
        "model_path": str(_model_path),
        "strategies": ic_results,
    }
    # Best GNN strategy by mean_ic
    _best_strat = max(ic_results, key=lambda k: ic_results[k]["mean_ic"], default=None)
    if _best_strat:
        _ic_summary["best"] = {
            "strategy": _best_strat,
            **ic_results[_best_strat],
        }
    # Always write alongside checkpoint for auto_improve
    _default_out = _model_path.parent / "ic_results.json"
    for _dest in filter(None, [_out_path, _default_out]):
        try:
        ...
        break  # write to _out_path if given, else _default_out
    if _out_path and _out_path != _default_out:
        try:
            _out_path.parent.mkdir(parents=True, exist_ok=True)
            _out_path.write_text(_json_bt.dumps(_ic_summary, indent=2))
            print(f"  IC results → {_out_path}")
        except OSError as _e:
            log.warning("Could not write IC results to %s: %s", _out_path, _e)
