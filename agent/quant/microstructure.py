...
class SpreadEstimator(nn.Module):
    ...
    def corwin_schultz(self, high: torch.Tensor, low: torch.Tensor, variant: str = "CSD") -> torch.Tensor:
        """
...
class OrderFlowImbalance(nn.Module):
    ...
    def compute_ofi_trade_tape(
        self, prices: torch.Tensor, volumes: torch.Tensor, window_size: int = 12
    ) -> torch.Tensor:
...
class VPINCalculator(nn.Module):
    ...
    def bucket_trades(self, trades: list[tuple[float, float, int]]) -> list[VolumeBucket]:
...
class KyleLambdaEstimator:
    ...
    def estimate_lambda(self, prices: np.ndarray, signed_volume: np.ndarray, window_hours: int = 720) -> np.ndarray:
...
class MicrostructureFeatureExtractor(nn.Module):
    ...
    def forward(
        self, ohlcv: dict[str, torch.Tensor], trades: list | None = None, precomputed_lambda: torch.Tensor | None = None
    ) -> dict[str, torch.Tensor]:
        """Extract all available M9 features."""
        features = {}

        if "high" in ohlcv and "low" in ohlcv:
            features["spread_cs"] = self.spread_estimator.corwin_schultz(ohlcv["high"], ohlcv["low"])
