...
def build_tool_registry(config: AgentConfig | None = None) -> ToolRegistry:
    ...
    registry.register(PolymarketTool(cache=cache))
    registry.register(
    ...
    registry.register(InsiderFilingsTool(cache=cache))
    registry.register(GDELTTool(cache=cache))
    registry.register(CFTCTool(cache=cache))
    registry.register(WhaleAlertTool(cache=cache))
    registry.register(Form144Tool(cache=cache))
    registry.register(FinraShortVolumeTool(cache=cache))
    registry.register(PowerGridTool(cache=cache))
    registry.register(WikipediaPageviewsTool(cache=cache))
    registry.register(AISVesselTool(cache=cache))
    registry.register(RegulatoryGazetteTool(cache=cache))
    registry.register(WeatherAlertsTool(cache=cache))
    registry.register(EarthquakeProximityTool(cache=cache))
    registry.register(TransportThroughputTool(cache=cache))
    registry.register(DefiFlowsTool(cache=cache))
    registry.register(GovContractsTool(cache=cache))
    registry.register(AcademicPreprintsTool(cache=cache))
    registry.register(SanctionsMonitorTool(cache=cache))
    registry.register(CertTransparencyTool(cache=cache))
    registry.register(BankruptcyCourtTool(cache=cache))
    registry.register(DnsMonitorTool(cache=cache))
    registry.register(SovereignDebtTool(cache=cache))
    registry.register(CentralBankBalanceTool(fred_api_key=fred_key, cache=cache))
    registry.register(FoiaRequestsTool(cache=cache))
    registry.register(CreditorFilingsTool(cache=cache))
    registry.register(ComtradeTool(cache=cache))
    ...
    registry.register(CapitalFlowsTool(fred_api_key=fred_key, cache=cache))
    registry.register(PatentFilingsTool(cache=cache))
    registry.register(LobbyingTool(cache=cache))
    registry.register(SatelliteActivityTool(cache=cache))
    registry.register(ElectricityMonitorTool(cache=cache))
    registry.register(InterconnectionQueueTool(cache=cache))
    registry.register(DiseaseSurveillanceTool(cache=cache))
    registry.register(FoodSecurityTool(cache=cache))
    registry.register(PoliticalRiskTool(cache=cache))
    registry.register(InternetOutagesTool(cache=cache))
