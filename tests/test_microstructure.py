...
class TestSpreadEstimator:
    ...
    def test_roll_measure(self):
        ...
        assert 0.001 < estimated_spread < 0.1, f"Roll estimate {estimated_spread:.4f} not in reasonable range"
...
class TestVPINCalculator:
    ...
    def test_vpin_balanced_buckets(self):
        ...
        assert 0.1 < mean_vpin < 0.5, f"Balanced flow should give moderate VPIN (0.1-0.5), got {mean_vpin:.3f}"
    ...
    def test_vpin_imbalanced_buckets(self):
        ...
        mean_vpin = vpin.mean().item()
        assert mean_vpin > 0.5, f"Imbalanced flow should give high VPIN, got {mean_vpin:.3f}"
...
class TestKyleLambdaEstimator:
    ...
    def test_kyle_lambda_zero_volume(self):
        ...
        assert np.abs(lambda_estimates).max() < 0.01, "Zero volume should give near-zero lambda"
...
class TestMicrostructureFeatureExtractor:
    ...
    def test_feature_extraction_ohlcv_only(self):
        ...
        ohlcv = {"high": prices * 1.01, "low": prices * 0.99, "close": prices, "volume": torch.ones(T) * 10000.0}
