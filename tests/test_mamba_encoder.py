...
class TestMambaEncoderConstruction:
    ...
    def test_has_mamba_true_when_mambapy_available(self, encoder):
        """When mambapy IS installed, the encoder must report it.

        This asserts a fact about the environment, not about behaviour, so it
        has to establish that fact first. mambapy lives in the [ml] extra and
        CI installs only .[dev,quant], so without the guard this failed on
        every CI run with a bare `assert False is True`.
        """
        pytest.importorskip("mambapy", reason="mambapy not installed (see [ml] extra)")
